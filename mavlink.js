...
var mavlink = function(sysid, compid, version, definitions) {
	...
	if (definitions && !Array.isArray(definitions)) {
		//Load from previously generated definitions.
		//this.efinitions and this.enums are not loaded.
		this.loadDefinitionsFromObject(definitions);
	} else {
...
mavlink.prototype.loadDefinitionsFromObject = function(definition) {
	//this.definitions and this.enums are not loaded.
	this.messagesByID = definition.messagesByID;
	for (var j = 0; j < this.messagesByID.length; j++) {
		//If we're v1.0 then calculate message checksums
		if (this.version == "v1.0") {
			if (this.messagesByID[j])
				this.messageChecksums[this.messagesByID[j].$.id] = this.calculateMessageChecksum(this.messagesByID[j]);
		}
	}
	this.messagesByName = definition.messagesByName;
	this.enumsByName = definition.enumsByName;
	this.enumsByValue = definition.enumsByValue;
...
mavlink.prototype.bufferField = function(buf, offset, field, value) {
	...
	for (var i = 0; i<valueArr.length; i++) {
		...
		switch (fieldSplit[0]){
			...
			case 'int64_t':
				console.warn("No 64-bit Integer support yet!", field.$.name);
	...
	for (var i = 0; i<fields.length; i++) {
		...
		for (var j = 0; j<fields[i].arrayLength; j++) {
			...
			switch (fieldTypeName){
				...
				//TODO: Add support for the 64bit types
				case 'int64_t':
					console.warn("No 64-bit Integer support yet!", fields[i].$.name);
					//buf.writeFloatLE(value[i],offset);
					break;
				case 'uint64_t':
...
mavlink.prototype.createMessage = function(msgid, data, cb) {
	...
	for (var i = 0; i < message.field.length; i++) {
      ...
      if (data[message.field[i].$.name] === undefined) {
			...
			if (message.$.name !== 'COMMAND_INT' && message.$.name !== 'COMMAND_LONG')
				return;
		}
