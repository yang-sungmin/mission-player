...
var missionExtract = require('./mission_extract').processLog;
...
function processLog(filename) {
  ...
  rd.on('close', function() {
    ...
    for (var i = 1 ; i < log.length; i++) {
      if (new Date(log[i].LoggingTime) - new Date(log[i-1].LoggingTime) > 1 * 60 * 1000) { //separate if gap > 1 mins
